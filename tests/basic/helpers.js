...
async function basicReactiveAddAndRemoveTasks(page) {
    const taskCount = parseFloat(process.env.TASK_COUNT || 20);
    await addAndRemoveTasks({ page, reactive: true, taskCount });
}

async function basicNonReactiveAddAndRemoveTasks(page) {
    const taskCount = parseFloat(process.env.TASK_COUNT || 20);
    await addAndRemoveTasks({ page, reactive: false, taskCount });
}

module.exports = {
    basicReactiveAddAndRemoveTasks,
    basicNonReactiveAddAndRemoveTasks,
}
